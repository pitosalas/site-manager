...
//
// $Id: function.url.php,v 1.1 2007/01/09 10:32:38 alg Exp $
//
...
function smarty_function_url($params, &$smarty)
{
    ...
    } else
    {
	    ...
	    if (isset($params['folder'])) $fid = $params['folder']->id;
	    if (isset($params['item'])) $iid = $params['item']->id;
		if (isset($params['news'])) $niid = $params['news']['id'];
			    
	    $fidP = (isset($params['folder']) ? '&fid=' . $params['folder']->id : '');
	    ...
	    if ($type == 'opml')
	    {
	    	if (isset($fid))
	    	{
	    		...
	    		if ($f->dynamic == 0) $url = FOLDER_URL . $fid . '.opml'; else
	    	...
	    	} else if (isset($iid))
	    	{
				...
				if ($i->dynamic == 0) $url = ITEM_URL . $iid . '.opml'; else
				$url = $i->dataURL;
	    	}
	    } else
	    if ($type == 'rss')
	    {
			...
			if ($i->dynamic == 0) $url = ITEM_URL . $iid . '.xml'; else
			$url = $i->dataURL;
	    } else
		...
		{
	    	// No type specified
	        if (isset($iid)) $url = ITEM_URL . $iid; else
	        if (isset($fid)) $url = FOLDER_URL . $fid; else
