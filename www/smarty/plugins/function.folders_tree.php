...
//
// $Id: function.folders_tree.php,v 1.1 2007/01/09 10:32:38 alg Exp $
//
 ...
 */
function _folders_tree($folder, $perm_edit_content, $userId, &$smarty, $outstripes, $perm_edit_others_content)
{
    $txt = "";
    $cnt = 0;
    ...
    if (isset($folder->items))
    {
        foreach ($folder->items as $item)
        {
            ...
            $txt .= '<a href="' . smarty_function_url(array('item' => $item, 'type' => 'rss'), $smarty) . '">' . _css_image('rss', 'Data Feed') . '</a>';
